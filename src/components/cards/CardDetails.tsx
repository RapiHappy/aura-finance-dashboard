    ...
    <div className="flex-1 min-w-[320px] max-w-[500px] p-8 border border-white/[0.03] rounded-3xl bg-[#030303] shadow-[inset_0_1px_0_rgba(255,255,255,0.02)] flex flex-col stagger-2">
      <div className="flex justify-between items-center mb-8">
        <h3 className="text-[11px] font-mono uppercase tracking-widest text-zinc-500 flex items-center gap-2">
          <Settings2 size={14} /> {t.cardSettings}
        </h3>
        <Badge variant={currentStatus === 'Active' ? 'success' : 'default'}>{currentStatus === 'Active' ? t.active : t.frozen}</Badge>
      </div>

      {/* Spend Limit Progress */}
      <div className="flex flex-col gap-4 mb-10">
        <div className="flex justify-between items-end">
          <span className="text-[13px] font-medium text-white tracking-tight">{t.monthlyLimit}</span>
        ...
        <div className="flex justify-between text-[11px] text-zinc-500 tracking-wide">
          ...
          <button className="text-indigo-400 hover:text-indigo-300 spring-transition">{t.editLimit}</button>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 mb-auto">
        <div className="flex justify-between items-center py-3 border-b border-white/[0.02]">
          <span className="text-[13px] text-zinc-400">{t.cardholder}</span>
        ...
        <div className="flex justify-between items-center py-3 border-b border-white/[0.02]">
          <span className="text-[13px] text-zinc-400">{t.cardNumber}</span>
            ...
            <span className="text-[13px] font-mono text-white tracking-widest flex items-center gap-2">
              {isRevealing ? (
                <span className="flex items-center gap-2 text-indigo-400">
                  <Loader2 size={14} className="animate-spin" /> {t.decrypting}
                </span>
              ) : isRevealed ? (
        ...
        <div className="flex justify-between items-center py-3 border-b border-white/[0.02]">
          <span className="text-[13px] text-zinc-400">{t.billingAddress}</span>
      ...
      <div className="flex gap-3 mt-8">
        ...
        >
          ...
          {isRevealing ? t.decrypting : isRevealed ? t.hideDetails : t.revealDetails}
        </motion.button>
        ...
        {currentStatus === 'Active' ? (
          ...
          >
            ...
            {isFreezing ? t.unfreezing : t.unfreeze}
          </motion.button>
        )}
      </div>
    </div>
...
export function CardTransactions({ card }: { card: CorporateCard }) {
  ...
  return (
    <div className="flex-[2] min-w-[400px] p-8 border border-white/[0.03] rounded-3xl bg-[#030303] shadow-[inset_0_1px_0_rgba(255,255,255,0.02)] flex flex-col stagger-3">
      <div className="flex justify-between items-center mb-8">
        <h3 className="text-[11px] font-mono uppercase tracking-widest text-zinc-500 flex items-center gap-2">
          <Activity size={14} /> {t.recentActivity}
        </h3>
        <button 
          onClick={() => window.location.href = '/transactions'}
          className="text-[13px] text-zinc-500 hover:text-white spring-transition flex items-center gap-1"
        >
          {t.viewLedger} <ArrowUpRight size={14} className="lucide lucide-arrow-up-right" />
        </button>
      </div>

      <div className="flex flex-col gap-2">
        {recent.map(txn => (
          <div key={txn.id} className="p-4 border border-white/[0.02] rounded-2xl flex items-center justify-between hover:bg-white/[0.02] spring-transition cursor-pointer group">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[13px] font-medium text-white tracking-tight">{txn.merchant}</p>
                <p className="text-[11px] text-zinc-500 tracking-wide mt-0.5">{txn.date}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-[14px] font-mono tracking-tighter text-zinc-300">{txn.amount}</p>
